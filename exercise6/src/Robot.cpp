...
Robot::Robot(std::string PLANNING_GROUP) : move_group_arm(PLANNING_GROUP+"_arm"), moveit_visual("base_link"), state(getCurrentState()) {
...
robot_state::RobotState Robot::getStateFromJointValues(const std::array<double, 7> &joint_values) {
    ...
    for (size_t i = 0; i < joint_values.size(); ++i) {
        s.setVariablePosition(std::string(planning_group+"_joint") + std::to_string(i + 1), joint_values[i]);
    }
    return s;
}
...
moveit_msgs::RobotTrajectory
Robot::planToPose(const robot_state::RobotState &start,const geometry_msgs::Pose &goal){
    move_group_arm.setStartState(start);
    if (!move_group_arm.setPoseTarget(goal)) {
        ROS_ERROR_STREAM("Cannot set joint value target for planning");
        return moveit_msgs::RobotTrajectory();
    }
    moveit::planning_interface::MoveGroupInterface::Plan plan;
...
void Robot::visualiseTrajectory(const moveit_msgs::RobotTrajectory &trajectory) {
    moveit_visual.publishTrajectoryLine(trajectory, state.getLinkModel(planning_group+"_ee"),
                                        state.getJointModelGroup(planning_group+"_arm"));
    moveit_visual.trigger();
}
